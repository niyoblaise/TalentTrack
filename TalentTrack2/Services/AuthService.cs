...
namespace TalentTrack2.Services
{
    public class AuthService : IAuthService
    {
        ...
        public async Task<bool> SendOtpAsync(string userId)
        {
            ...
            {
                // Log the error (if logger was available) or just ignore for now to prevent 500 crash
                // In a real app, we should log this.
                Console.WriteLine($"Failed to send email: {ex.Message}");
                // Return false or throw? For now, let's return false so the frontend knows something went wrong, 
                // but ideally we should distinguish between "OTP generated" and "Email sent".
                // However, to fix the immediate 500 crash, let's return false here.
                return false; 
            }
            return true;
        }
